...
CrateCheckPoint::CrateCheckPoint(GameObject* crashBandicoot, DirectX::XMFLOAT3 cratePos)
	:m_Pos(cratePos)
{
...
void CrateCheckPoint::Initialize(const GameContext& gameContext)
{
	//-- Essentials --//
	...
	//-- MATERIALS --//
	if (!gameContext.pMaterialManager->GetMaterial(5004))
	{
		auto pNormalCrateMat = new DiffuseMaterial();
		pNormalCrateMat->SetDiffuseTexture(L"Resources/Game/Texture/crate_checkpoint.bmp");
		gameContext.pMaterialManager->AddMaterial(pNormalCrateMat, 5004);
	}
	...
	m_pNormalBox->GetComponent<ModelComponent>()->SetMaterial(5004);
	...
	m_pSoundManager->GetSystem()->createSound("./Resources/Game/Sound/SFX/CheckPoint.mp3", FMOD_LOOP_OFF | FMOD_2D, 0, &m_pSound);
}

void CrateCheckPoint::Update(const GameContext& )
{
	...
	if (m_IsTriggeredTop)
	{
		static_cast<CrashBandicoot*>(m_pCrashBandicoot)->SetRespawnTarget(m_Pos);
...
void CrateCheckPoint::RemoveAllObjects()
{
	...
	//-- OPENED CHECKPOINT BOX --//
	m_pCheckPointBox = new GameObject{};
	m_pCheckPointBox->AddComponent(new ModelComponent{ L"Resources/Game/Models/CheckPoint.ovm",true });
	m_pCheckPointBox->AddComponent(new RigidBodyComponent());
	m_pCheckPointBox->GetComponent<RigidBodyComponent>()->SetKinematic(true);
	std::shared_ptr<physx::PxGeometry> geomCC = std::make_shared<physx::PxBoxGeometry>();
	auto colliderCC = new ColliderComponent(geomCC, *pDefaultMaterial);
	m_pCheckPointBox->AddComponent(colliderCC);
	m_pCheckPointBox->GetTransform()->Scale(0.3f, 0.3f, 0.3f);
	m_pCheckPointBox->GetTransform()->Translate(m_Pos);
	m_pCheckPointBox->GetTransform()->Rotate(180.0f, 0.0f, 0.0f);
	m_pCheckPointBox->GetComponent<ModelComponent>()->SetMaterial(5004);
	SceneManager::GetInstance()->GetActiveScene()->AddChild(m_pCheckPointBox);
