...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		vertexBuffer.ByteWidth = m_BufferSize * sizeof(SpriteVertex);
		...
		HRESULT hrResult = gameContext.pDevice->CreateBuffer(&vertexBuffer, nullptr, &m_pVertexBuffer);
		Logger::LogInfo(std::to_wstring(hrResult));
	}
	...
	D3D11_MAPPED_SUBRESOURCE mapResc{};
	HRESULT hrResult = gameContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mapResc);
	memcpy(mapResc.pData, m_Sprites.data(), m_Sprites.size() * sizeof(SpriteVertex));
