...
VS_OUTPUT VS(VS_INPUT input){
	...
	//After the steps above, we can calculate the output normal and position.
	...
	for (int i = 0; i < 4; ++i)
	{
		int currentBoneId = input.blendIndices[i];
		if (currentBoneId > -1)
		{
			transformedPosition += mul(originalPosition, gBones[currentBoneId]) * input.blendWeights[i];
