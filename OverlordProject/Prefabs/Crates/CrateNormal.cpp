...
CrateNormal::CrateNormal(GameObject* crashBandicoot, DirectX::XMFLOAT3 cratePos)
	:m_Pos(cratePos)
{
...
void CrateNormal::Initialize(const GameContext& gameContext)
{
	//-- Essentials --//
	...
	//-- MATERIALS --//
	if (!gameContext.pMaterialManager->GetMaterial(5001))
	{
		auto pNormalCrateMat = new DiffuseMaterial();
		pNormalCrateMat->SetDiffuseTexture(L"Resources/Game/Texture/crate_question.bmp");
		gameContext.pMaterialManager->AddMaterial(pNormalCrateMat, 5001);
	}
	...
	m_pNormalBox->GetComponent<ModelComponent>()->SetMaterial(5001);
...
void CrateNormal::Update(const GameContext& )
{
...
void CrateNormal::RemoveAllObjects()
{
...
void CrateNormal::SetWumpaFruits()
{
	PlayerInventory::GetInstance()->SetSmashedBoxes(1);
	for (unsigned int i = 0; i < 5; i++)
	{
		WumpaFruit* fruit = new WumpaFruit{ m_pCrashBandicoot };
		m_pWumpaFruits.push_back(fruit);
		SceneManager::GetInstance()->GetActiveScene()->AddChild(m_pWumpaFruits[i]);
	}

		m_pWumpaFruits[0]->GetTransform()->Translate(m_Pos.x, m_Pos.y + 3.f, m_Pos.z);
		m_pWumpaFruits[1]->GetTransform()->Translate(m_Pos.x + 2.0f, m_Pos.y + 3.f, m_Pos.z + 2.0f);
		m_pWumpaFruits[2]->GetTransform()->Translate(m_Pos.x - 2.0f, m_Pos.y + 3.f, m_Pos.z - 2.0f);
		m_pWumpaFruits[3]->GetTransform()->Translate(m_Pos.x - 2.0f, m_Pos.y + 3.f, m_Pos.z + 2.0f);
		m_pWumpaFruits[4]->GetTransform()->Translate(m_Pos.x + 2.0f, m_Pos.y + 3.f, m_Pos.z - 2.0f);
}
